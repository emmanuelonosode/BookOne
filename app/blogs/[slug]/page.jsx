    ...
    image: ({ value }) =>
      value?.asset ? (
        <figure className="my-10">
          ...
          {value.caption && (
            <figcaption className="text-center text-xs text-[#6F6A62] mt-3 font-mono tracking-wide">
              {value.caption}
            </figcaption>
          )}
        </figure>
      ...
      return (
        <div className="my-8 overflow-x-auto border border-[#1C1917]/[0.08]">
          <table className="min-w-full">
            {value.caption && (
              <caption className="text-xs text-[#6F6A62] px-4 py-3 border-b border-[#1C1917]/[0.08] font-mono text-left">
                {value.caption}
              </caption>
            )}
            <tbody>
  ...
  block: {
    h1: ({ children }) => (
      <h1 id={String(children?.[0] ?? "").replace(/\s+/g, "-").toLowerCase()}
        className="font-display font-semibold text-[#1C1917] leading-tight tracking-[-0.01em] mt-12 mb-5 scroll-mt-24"
    ...
    normal: ({ children }) => (
      <p className="mb-6 text-[1.0625rem] leading-[1.75] text-[#3A352F]">{children}</p>
    ),
    blockquote: ({ children }) => (
  ...
  listItem: {
    bullet: ({ children }) => (
      <li className="flex items-start gap-3">
        ...
        <span className="text-[1.0625rem] text-[#3A352F] leading-relaxed">{children}</span>
      </li>
    ),
    number: ({ children }) => (
      <li className="flex items-start gap-3">
        ...
        <div className="pt-32 pb-14 border-b border-[#1C1917]/[0.08]">
          <div className="max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
            ...
            <div className="grid lg:grid-cols-[1fr_280px] gap-16 items-start">
              <div>
                {category && (
                  <p className="text-[10px] tracking-[0.25em] text-[#6F6A62] uppercase font-mono mb-4">{category}</p>
                )}
                <h1 className="font-display font-medium text-[#1C1917] leading-[1.05] tracking-[-0.02em] mb-6"
                  style={{ fontSize: "clamp(2.2rem, 5vw, 4.5rem)" }}>
                  {blog.title}
                </h1>
                {blog.description && (
                  <p className="text-lg text-[#6F6A62] leading-relaxed max-w-2xl mb-8">{blog.description}</p>
                )}

                {/* Meta row */}
                <div className="flex flex-wrap items-center gap-6 text-[10px] tracking-[0.15em] uppercase text-[#6F6A62] font-mono">
                  {blog.author?.name && (
              ...
              <div className="hidden lg:block">
                <p className="text-[10px] tracking-[0.2em] uppercase text-[#6F6A62] font-mono mb-4">Share</p>
        ...
        <div className="max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16 py-16">
          <div className="grid lg:grid-cols-[220px_1fr] gap-16 items-start">
            ...
            <aside className="hidden lg:block lg:sticky lg:top-28">
              <p className="text-[10px] tracking-[0.2em] uppercase text-[#6F6A62] font-mono mb-4">Contents</p>
            ...
            <article className="min-w-0">
              ...
              {blog.tags?.length > 0 && (
                <div className="mt-12 pt-8 border-t border-[#1C1917]/[0.08]">
                  <p className="text-[10px] tracking-[0.2em] uppercase text-[#6F6A62] font-mono mb-4">Tags</p>
                  <div className="flex flex-wrap gap-2">
                    {blog.tags.map((tag, i) => (
                      <span key={i}
                        className="px-3 py-1 border border-[#1C1917]/[0.08] text-[10px] tracking-[0.1em] uppercase font-mono text-[#6F6A62]">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              {blog.author && (
                <div className="mt-12 pt-8 border-t border-[#1C1917]/[0.08]">
                  <div className="flex items-start gap-6">
                    {blog.author.image && (
                      <div className="w-14 h-14 rounded-full overflow-hidden bg-[#1C1917]/[0.04] shrink-0 ring-1 ring-[#1C1917]/10">
                        <Image
              ...
              {/* Share bottom */}
              <div className="mt-12 pt-8 border-t border-[#1C1917]/[0.08]">
                <p className="text-[10px] tracking-[0.2em] uppercase text-[#6F6A62] font-mono mb-4">Share this article</p>
                <ShareButtons url={currentUrl} title={blog.title} />
              </div>
